...
void jogo(int jump)
{
    ...
    printf("%s",linha9);
    gotoxy(1,27-w);
    // C�DIGO PARA A MOVIMENTA��O DAS PERNAS
    //Posi��o das pernas no ar:
    if(jump==1 || jump==2)
    {
        printf("%s", linha9);
        gotoxy(1,27-w);
        printf("%s",linha10);
        //Alternan�a da posi��o das pernas ao correr:
    }
    else if(jump == 0 && c == 1)
    {
        gotoxy(1,27-w);
    ...
    gotoxy(1,28-w);
    //Se n�o estiver no ch�o, imprime vazio logo abaixo do dino
    printf("                ");
    delay(spd);
...
char obstaculo()
{
    ...
    if(z>=68 && w<3)//CASO AS COORDENADAS DO T-REX COINCIDAM COM A DO OBSTACULO
    {
        ...
        gotoxy(16,9);
        printf("                                                             ");//APAGA A MENSAGEM DE CONTINUA��O
